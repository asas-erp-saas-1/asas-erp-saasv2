...
export function StrategicForecastingModule() {
  ...
  useEffect(() => {
    async function load() {
      setLoading(true);
      try {
        const res = await fetch(`/api/intelligence?bankDelayDays=60&stressScenario=${useStress}&useAi=true`);
        const json = await res.json();
        
        // Map from API model to chart model
        if (json.metrics?.treasuryForecast) {
          const mapped = json.metrics.treasuryForecast.map((d: any) => ({
             name: d.monthName,
             revenue: d.predicted_inflow / 1000000, 
             target: (d.predicted_inflow + (d.delayed_impact||0)) / 1000000,
             prediction: d.expected_balance / 1000000,
             cost: d.predicted_outflow / 1000000,
          }));
    ...
    >
      ...
      <motion.div variants={itemVariants} className="flex flex-col md:flex-row md:items-end justify-between gap-3 sm:gap-4 py-2 border-b border-white/5 pb-4 sm:pb-6">
        ...
        <div className="flex flex-row items-center gap-2 sm:gap-3 w-full md:w-auto mt-2 sm:mt-0">
          ...
          <button onClick={() => setUseStress(!useStress)} className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 sm:gap-2 px-3 sm:px-6 py-2.5 bg-asas-gold hover:bg-[#E0B96B] text-[#051121] rounded-xl text-[9px] sm:text-[10px] font-bold uppercase tracking-widest transition-all shadow-[0_0_15px_rgba(212,166,79,0.3)] hover:shadow-[0_0_20px_rgba(212,166,79,0.5)]">
            <RefreshCcw className={`w-3 h-3 sm:w-3.5 sm:h-3.5 hidden sm:block ${loading ? 'animate-spin' : ''}`} /> {useStress ? 'Stress Active' : 'Simulation'}
          </button>
        </div>
      </motion.div>
