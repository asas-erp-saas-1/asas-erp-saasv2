...
export function NextMobileMenu({ profile, initial, roleDisplay }: { profile: any, initial: string, roleDisplay: string }) {
  ...
  return (
    <>
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-[9990] bg-[#0A0A0A]/95 backdrop-blur-3xl border-t border-white/5 pb-[env(safe-area-inset-bottom)] shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
      ...
      <AnimatePresence>
        {isOpen && (
          <div key="mobile-menu-overlay" className="md:hidden">
            <motion.div 
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              onClick={() => setIsOpen(false)}
              className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[9995]"
            />
            <motion.div
              initial={{ y: '100%' }}
              animate={{ y: 0 }}
              ...
              className="fixed inset-x-0 bottom-0 top-[12%] bg-[#0A0A0A] rounded-t-[2rem] border-t border-white/10 z-[9999] flex flex-col shadow-[0_-20px_50px_rgba(0,0,0,0.7)] overflow-hidden"
