...
export default async function InvitePage({ params }: { params: { code: string } }) {
  ...
  if (!user) {
    ...
    return (
      <div className="flex min-h-[100dvh] w-full items-center justify-center bg-gray-50 dark:bg-[#050505] text-gray-900 dark:text-white px-4 py-12 bg-dot-grid">
         <div className="w-full max-w-md p-8 border border-gray-200 dark:border-white/5 bg-white dark:bg-[#0A0A0A] rounded-3xl shadow-2xl text-center">
            ...
            <div className="space-y-4">
              ...
              <Link href={`/signup?invite=${params.code}`} className="block w-full px-6 py-3 bg-gray-100 hover:bg-gray-200 dark:bg-[#111111] dark:hover:bg-[#1a1a1a] text-gray-900 dark:text-white rounded-xl font-bold border border-gray-200 dark:border-white/10 transition-all">
                Créer un compte
              </Link>
            </div>
         </div>
