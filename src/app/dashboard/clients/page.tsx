  ...
  return (
    <div className="flex-1 font-sans text-gray-100 flex flex-col">
      <div className="w-full space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6">
          <div className="relative flex-1 max-w-md">
            ...
            <input type="text" placeholder="Rechercher entité..." value={search}
              ...
              className="w-full pl-11 pr-4 py-3 text-sm font-medium border border-white/10 rounded-xl bg-[#050505] text-white focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all placeholder:text-gray-600" />
          </div>
          <select value={type} onChange={e => setType(e.target.value)}
            className="px-5 py-3 text-sm font-medium border border-white/10 rounded-xl bg-[#050505] text-white focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all appearance-none pr-10 relative cursor-pointer min-w-[160px]">
        ...
        {/* Data */}
        <div className="bg-[#050505] rounded-[2rem] border border-white/5 shadow-2xl overflow-hidden">
        {loading ? (
          <div className="p-6 space-y-3">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="h-16 bg-[#0A0A0A] border border-white/5 rounded-2xl animate-pulse" />
            ))}
          </div>
        ) : clients.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-24 text-gray-500">
               <div className="w-20 h-20 bg-[#0A0A0A] border border-white/5 rounded-3xl flex items-center justify-center mb-6">
                  <FileUser className="h-10 w-10 text-gray-600" />
               </div>
              <p className="font-extrabold text-white text-lg">Base de données vide</p>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-white/5">
                  <AnimatePresence>
                  {clients.map((c, i) => {
                     const cfg = TYPE_CONFIG[c.type] ?? { color: 'bg-white/5 border-white/10 text-gray-300', label: c.type };
                     return (
                        <motion.tr 
                        ...
                        <td className="px-6 py-5">
                           <p className="text-sm font-bold text-white group-hover:text-blue-400 transition-colors">{c.full_name}</p>
                           <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mt-1.5 font-mono">ID: {c.id.slice(0, 8)}</p>
                        </td>

                        {/* Contact */}
                        <td className="px-6 py-5 space-y-2">
                           {c.phone && <div className="flex items-center gap-3 text-xs font-bold text-gray-300"><Phone className="h-3.5 w-3.5 text-gray-500" />{c.phone}</div>}
                           {c.email && <div className="flex items-center gap-3 text-xs font-bold text-gray-300"><Mail className="h-3.5 w-3.5 text-gray-500" />{c.email}</div>}
                        ...
                        {/* Source */}
                        <td className="px-6 py-5 text-[10px] uppercase tracking-widest font-bold text-gray-400 align-middle">
                           ...
                           <div className="flex items-center gap-3">
                              {c.nationality ? (
                                 <>
                                    ...
                                    <span className="text-xs font-bold text-gray-300 uppercase tracking-widest">{c.nationality}</span>
                                 </>
                        ...
                        <td className="pr-6 align-middle text-right">
                           <ChevronRight className="h-5 w-5 text-gray-600 transition-transform group-hover:text-white group-hover:translate-x-1" />
                        </td>
                        </motion.tr>
                     )
                   })}
