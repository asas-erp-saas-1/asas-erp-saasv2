  ...
  return (
    <div className="flex min-h-[100dvh] w-full items-center justify-center bg-gray-50 dark:bg-[#050505] text-gray-900 dark:text-white px-4 py-12 bg-dot-grid relative">
      ...
      <div className="w-full max-w-md p-6 sm:p-8 border border-gray-200 dark:border-white/5 bg-white dark:bg-[#0A0A0A] rounded-3xl shadow-2xl z-10 relative">
        ...
        <p className="text-gray-600 dark:text-gray-400 text-center text-sm mb-8">Rejoignez ASAS OS pour digitaliser votre agence</p>
        ...
        <form onSubmit={handleSignup} className="flex flex-col gap-4">
          <div>
            <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1.5 uppercase tracking-wide">Nom Complet</label>
            <input
              type="text"
              required
              value={fullName}
        ...
        <div className="mt-6 text-center text-sm font-medium text-gray-500">
           Déjà un compte ? <button onClick={() => router.push('/login')} className="text-blue-500 hover:text-blue-400">Connectez-vous</button>
        </div>
      </div>
    </div>
  );
